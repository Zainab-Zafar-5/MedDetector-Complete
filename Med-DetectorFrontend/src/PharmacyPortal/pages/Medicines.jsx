...
const Medicines = () => {
  ...
  const fetchPage = useCallback(async (page, search = "") => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/medicines?page=${page}${searchQuery}`, {
        method: "GET",
        headers: {
  ...
  // ✅ SEARCH: debounced so it doesn't fire an API call on every keystroke.
  // Always resets to page 1, since a new search term can have a different
  // total number of results/pages than the unfiltered list.
  useEffect(() => {
      ...
      <div className="main-content" style={{ flex: "1 1 0%", minWidth: 0, width: "100%" }}>
        ...
        <div className="page-content" style={{ padding: "30px", width: "100%", maxWidth: "100%", boxSizing: "border-box" }}>
          ...
          <div style={{ marginBottom: "16px" }}>
            <input
              type="text"
              placeholder="🔍 Search by medicine name or generic formula..."
              value={searchTerm}
                    ...
                    {medicines.length === 0 ? (
                      <tr>
                        <td colSpan="8" style={{ textAlign: "center", padding: "30px", color: "#64748b" }}>
                          {searchTerm ? `No medicines found matching "${searchTerm}".` : "No tenant data nodes allocated in this tracking boundary set. Import CSV sheet!"}
                        </td>
                      </tr>
                    ) : (
