...
import { Search as SearchIcon, MapPin, Phone, Loader2, Pill, Info, X, Mail, ShieldCheck, Upload, Clock, ShoppingBag } from 'lucide-react';
import styles from './SearchResults.module.css';
import Alternative from './Alternative';
import OrderFormModal from "./OrderFormModal";
import { useNavigate, useSearchParams } from 'react-router-dom';

// ✅ FIX: Centralized, env-based API URL instead of hardcoded localhost.
// Set REACT_APP_API_URL in your .env file (e.g. https://api.meddetector.com)
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const data = await new Promise((resolve, reject) => {
        ...
        xhr.open('POST', `${API_BASE_URL}/api/prescriptions/upload`);
...
const OrderConfirmationModal = ({ data, onClose }) => {
  ...
  const handleSearch = async (val) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/search?q=${encodeURIComponent(val)}${cityParam}`);
      const resultJson = await response.json();
      
      if (resultJson.success && Array.isArray(resultJson.data)) {
        const searchTerm = val.trim().toLowerCase();
        const relevantData = resultJson.data.filter((item) => {
        ...
        const uniqueData = relevantData.filter((item, index, self) =>
  ...
  // ✅ GPS — full address detect karo
  const detectLocation = () => {
    if (!navigator.geolocation) {
      alert('GPS tumhare browser mein supported nahi hai.');
      return;
    }
    setGpsLoading(true);
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          // Full address: suburb/neighbourhood + city
          const parts = [
          ...
          alert('Location detect nahi ho saki. Please manually enter karo.');
        } finally {
          setGpsLoading(false);
        }
      },
      () => {
        alert('Location access deny. Please Enter manually');
        setGpsLoading(false);
      }
    );
  };
        ...
        <section className={styles.searchSection}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', padding: '14px 18px', borderBottom: '1px solid #f1f5f9' }}>
              <SearchIcon size={18} style={{ color: '#94a3b8', flexShrink: 0, marginRight: 12 }} />
              <input
                type="text"
                placeholder="Medicine name (e.g. Panadol, Paracetamol)..."
                value={query}
            ...
            <div style={{ display: 'flex', alignItems: 'center', padding: '12px 18px' }}>
              <MapPin size={17} style={{ color: '#94a3b8', flexShrink: 0, marginRight: 12 }} />
              <input
                type="text"
                placeholder="Area or city (e.g. Model Town, Lahore)..."
                value={city}
              ...
              {city && (
                <button onClick={() => setCity('')}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: 16, padding: '0 4px', marginRight: 8 }}>✕</button>
              )}
              <button
                ...
                style={{
                  flexShrink: 0,
                  background: gpsLoading ? '#f1f5f9' : 'linear-gradient(135deg, #3b5bdb, #60a5fa)',
            ...
            {city.trim() && (
              <div style={{
                ...
                <MapPin size={12} />
                Filtering by location: <strong style={{ marginLeft: 2 }}>{city}</strong>
                <button onClick={() => setCity('')}
                  style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', fontSize: 11 }}>
                  Remove filter
                </button>
              </div>
            )}
      ...
      {showModal && selectedMed && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
           ...
           <div className={styles.infoItemBoxed}>
  {/* Now using the pharmacyEmail field added by the updated API */}
  <Mail size={16} /> {selectedMed.pharmacyEmail || "No email available"}
